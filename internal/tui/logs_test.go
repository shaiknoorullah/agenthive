...
// NB: TestMain (forcing lipgloss into termenv.Ascii) is declared by
// actions_test.go, and the shared testStyles() helper is declared by
// routes_test.go. This file relies on both of those package-level
// helpers so every tab's golden file uses the same blank palette.
...
func TestLogsModel_View_GoldenAllLevels(t *testing.T) {
	tm := teatest.NewTestModel(
		t,
		logsHarness{inner: NewLogsModel(testStyles())},
...
func lastFrame(b []byte) []byte {
	s := string(b)
	// bubbletea uses the alt-screen restore sequence "\x1b[?1049h" at the
	// start and "\x1b[?1049l" at the end. Strip anything outside the
	// active screen so the golden does not depend on those bookends.
	...
	// Strip the final ANSI cursor-hide/show toggles for stability.
	s = strings.ReplaceAll(s, "\x1b[?25l", "")
	s = strings.ReplaceAll(s, "\x1b[?25h", "")
	// The interesting render is the trailing block after the final
	// cursor-home sequence. teatest's ANSI compressor collapses
	// duplicate frames, so the final block is what the user sees.
